...
def _load_duration_latency_clusters(
    cfg: ErpTopoVizConfig,
) -> tuple[np.ndarray, np.ndarray, list[tuple[np.ndarray, ...]], np.ndarray, np.ndarray, list[tuple[np.ndarray, ...]], float]:
    ...
    return duration_t, duration_p, duration_clusters, latency_t, latency_p, latency_clusters, data_tmin


def _save_topo_figure(fig: Any, output_path: Path) -> None:
    from matplotlib import pyplot as plt
    from turntaking.viz._style import save_figure

    output_path.parent.mkdir(parents=True, exist_ok=True)
    save_figure(fig, save_basepath=output_path.with_suffix(""), profile_name="jneuro_2col")
    plt.close(fig)
...
def _run_impl(cfg: ErpTopoVizConfig) -> None:
    ...
    duration_t, duration_p, duration_clusters, latency_t, latency_p, latency_clusters, data_tmin = (
        _load_duration_latency_clusters(cfg)
    )
    ...
    _save_topo_figure(fig_duration, cfg.out_duration)
    ...
    _save_topo_figure(fig_latency, cfg.out_latency)
